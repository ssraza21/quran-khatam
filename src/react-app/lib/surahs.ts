...
export const SURAHS: SurahOption[] = SURAH_NAMES.map((name, index) => ({
  number: index + 1,
  name,
}));

export function getSurahName(number: number | null | undefined): string {
  if (!number || number < 1 || number > SURAHS.length) return "Surah";
